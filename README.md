...
**FPGA Report** (`make report`): Generates a static analysis of how the design maps to FPGA, providing estimates of II, latency, and fMAX without running on hardware.

**FPGA Simulation** (`make fpga_sim`): Simulates the design on the actual FPGA chip. Requires Quartus/ModelSim licences.

**FPGA Build** (`make fpga`): Builds and deploys firmware onto a physical FPGA. The only mode where wall-clock time measurement is meaningful.

## Source

Intel hls-samples: https://github.com/altera-fpga/hls-samples
