...
// SystemVerilog created from DijkstraParallelID_function_cra_agent
...
module DijkstraParallelID_function_cra_agent (
    ...
    reg [191:0] pipe_reg0_kernel_arguments_q;
    reg [0:0] pipe_reg0_kernel_start_q;
    reg [191:0] pipe_reg1_kernel_arguments_q;
    ...
    // version_number(CONSTANT,122)
    assign version_number_q = 16'b0000000000000101;

    // running_bit(BITSELECT,95)
    ...
    // write_selector_start_status(MUX,124)
    ...
    // start_bit(BITSELECT,105)
    assign start_bit_b = start_status_reg_q[0:0];

    // start_or_start_buffered(LOGICAL,110)
    ...
    // buffered_start_NO_SHIFT_REG(REG,34)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // start_buffered_and_kernel_idle(LOGICAL,107)
    assign start_buffered_and_kernel_idle_q = not_running_bit_q & buffered_start_NO_SHIFT_REG_q;

    // start_from_buffered_start_NO_SHIFT_REG(REG,108)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // bit_enable_bar(LOGICAL,29)
    assign bit_enable_bar_q = ~ (bit_enable_q);

    // bit_enable_bottom_bar(BITSELECT,31)
    ...
    // adder_counter_width(BITSELECT,12)
    ...
    // status_self_update(BITJOIN,120)
    ...
    // printf_reset_bit(BITSELECT,89)
    assign printf_reset_bit_b = status_NO_SHIFT_REG_q[4:4];

    // acl_counter_reset(GPOUT,168)
    ...
    // readdata_upper_bits_mux(MUX,94)
    ...
    // readdata_lower_bits_mux(MUX,92)
    ...
    // dupName_5_address_ref_x(CONSTANT,151)
    assign dupName_5_address_ref_x_q = 5'b10010;

    // dupName_8_NO_NAME_x(LOGICAL,155)
    assign dupName_8_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_5_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_5_can_write_x(LOGICAL,152)
    assign dupName_5_can_write_x_q = dupName_8_NO_NAME_x_q & avs_cra_write;

    // bus_high(BITSELECT,35)
    assign bus_high_b = avs_cra_writedata[63:32];

    // bit_enable_top(BITSELECT,32)
    assign bit_enable_top_b = bit_enable_q[63:32];

    // dupName_0_mask1_x(LOGICAL,127)
    assign dupName_0_mask1_x_q = bit_enable_top_b & bus_high_b;

    // bit_enable_top_bar(BITSELECT,33)
    assign bit_enable_top_bar_b = bit_enable_bar_q[63:32];

    // dupName_4_mask0_x(LOGICAL,148)
    assign dupName_4_mask0_x_q = bit_enable_top_bar_b & arguments_5_buffered_q;

    // dupName_4_new_data_x(LOGICAL,150)
    assign dupName_4_new_data_x_q = dupName_4_mask0_x_q | dupName_0_mask1_x_q;

    // arguments_5_buffered(REG,27)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_3_mask0_x(LOGICAL,143)
    assign dupName_3_mask0_x_q = bit_enable_bottom_bar_b & arguments_4_buffered_q;

    // dupName_3_new_data_x(LOGICAL,145)
    assign dupName_3_new_data_x_q = dupName_3_mask0_x_q | Start_mask1_q;

    // arguments_4_buffered(REG,25)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_1_arg_bit_join_x(BITJOIN,136)
    assign dupName_1_arg_bit_join_x_q = {arguments_5_q, arguments_4_q};

    // dupName_4_address_ref_x(CONSTANT,146)
    assign dupName_4_address_ref_x_q = 5'b10001;

    // dupName_7_NO_NAME_x(LOGICAL,154)
    assign dupName_7_NO_NAME_x_q = $unsigned(avs_cra_address == dupName_4_address_ref_x_q ? 1'b1 : 1'b0);

    // dupName_4_can_write_x(LOGICAL,147)
    assign dupName_4_can_write_x_q = dupName_7_NO_NAME_x_q & avs_cra_write;

    // dupName_2_mask0_x(LOGICAL,138)
    assign dupName_2_mask0_x_q = bit_enable_top_bar_b & arguments_3_buffered_q;

    // dupName_2_new_data_x(LOGICAL,140)
    assign dupName_2_new_data_x_q = dupName_2_mask0_x_q | dupName_0_mask1_x_q;

    // arguments_3_buffered(REG,23)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // dupName_1_mask0_x(LOGICAL,133)
    assign dupName_1_mask0_x_q = bit_enable_bottom_bar_b & arguments_2_buffered_q;

    // dupName_1_new_data_x(LOGICAL,135)
    assign dupName_1_new_data_x_q = dupName_1_mask0_x_q | Start_mask1_q;

    // arguments_2_buffered(REG,21)
    always_ff @ (posedge clock)
    begin
        if (!resetn)
        begin
    ...
    // arg_bit_join(BITJOIN,15)
    assign arg_bit_join_q = {arguments_1_q, arguments_0_q};

    // kernel_arg_bit_join(BITJOIN,69)
    assign kernel_arg_bit_join_q = {dupName_1_arg_bit_join_x_q, dupName_0_arg_bit_join_x_q, arg_bit_join_q};

    // pipe_reg0_kernel_arguments(REG,82)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // pipe_reg1_kernel_arguments(REG,84)
    always_ff @ (posedge clock)
    begin
        if (0)
        begin
    ...
    // kernel_arguments(GPOUT,172)
    assign kernel_arguments = pipe_reg1_kernel_arguments_q;

    // will_be_started(LOGICAL,123)
    ...
    // start_is_or_going_high(LOGICAL,109)
