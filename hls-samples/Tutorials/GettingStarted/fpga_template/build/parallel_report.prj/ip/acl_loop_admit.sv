...
/*
  ...
  ******************************************************************************
  ***************************** !!!!!!WARNING!!!!!! ****************************
  * A software model of this IP exists in the acl-source repository, that attempts to model the latencies in the IP that are needed for setting constraints related to speculated iterations and reinvocation delay when scheduling
  * An update to AclLoopAdmit (name of source files for software model) is required whenever changes are made to this IP to prevent latencies from getting out of sync
  ******************************************************************************
  ******************************************************************************
  ******************************************************************************
...
module acl_loop_admit #(
    ...
    parameter int COMPILER_INFERRED_SPECULATED_ITERATIONS = 3,       // The number of speculated iterations calculated by the compiler. Default is calculated from formula in AclLoopAdmit with LATENCY_O_VALID_TO_I_VALID=1 and default params above
    parameter int COMPILER_INFERRED_REINVOCATION_DELAY = 5,          // The number of cycles of reinvocation delay calculated by the compiler. Default is calculated from formula in AclLoopAdmit with LATENCY_O_VALID_TO_I_VALID=1 and default params above
    parameter int LATENCY_O_VALID_TO_I_VALID = 1                     // The scheduled latency of the number of cycles it takes for the exit condition calculation. This is used for verifying that the compiler inferred speculation and reinvocation delay are correct
)
(
    input  wire     clock,
    input  wire     resetn,
  ...
  */
  ...
  // In FAST_ADMIT mode, thread_is_exiting asserts on the same cycle as real_exit_occurring
  localparam EXIT_LATENCY = FAST_ADMIT? 0 : 1;

  // In FAST_EXIT mode, inherent IP latency is reduced by 1 (Max number of cycles it takes for tt_ptr_admit_ahead to loop back to the same thread table slot.
  // FAST_EXIT allows exit and admit to happen in same cycle, when tt_ptr_admit_ahead == tt_ptr_exit and thread is exiting from thread table)
  localparam INHERENT_LATENCY = FAST_EXIT? II-1 : II;

  // Check that the IP latencies match what the compiler is using for scheduling the exit condition of the loop and achieving user speculated iterations or reinvocation delay
  `ACL_PARAMETER_ASSERT_MESSAGE(((FAST_ADMIT == 0 && TT_READ_LATENCY == 1 && TT_LATENCY_ADMIT_TO_FIRST_READ == 1 && LATENCY_ADMIT_DECISION_TO_OUT_STALL == 1 && EXIT_LATENCY == 1) ||
      (FAST_ADMIT == 1 && TT_READ_LATENCY == 0 && TT_LATENCY_ADMIT_TO_FIRST_READ == 0 && LATENCY_ADMIT_DECISION_TO_OUT_STALL == 0 && EXIT_LATENCY == 0)),
      "IP Latencies do not match compiler model")

  // Calculate the # of speculated iterations and verify that it matches the compiler
  `ACL_PARAMETER_ASSERT_MESSAGE((((TT_READ_LATENCY + my_max(OUTPUT_LATENCY_ELONGATE_AMOUNT,0) + EXIT_LATENCY + INHERENT_LATENCY - II + LATENCY_O_VALID_TO_I_VALID)/II) == COMPILER_INFERRED_SPECULATED_ITERATIONS),
      "Observed speculated iterations does not match the value calculated by the compiler")

    // Calculate the # of cycles of reinvocation delay and verify that it matches the compiler
  `ACL_PARAMETER_ASSERT_MESSAGE(((TT_READ_LATENCY + my_max(OUTPUT_LATENCY_ELONGATE_AMOUNT,0) + EXIT_LATENCY + INHERENT_LATENCY + TT_LATENCY_ADMIT_TO_FIRST_READ + LATENCY_O_VALID_TO_I_VALID) == COMPILER_INFERRED_REINVOCATION_DELAY),
      "Observed reinvocation delay does not match the value calculated by the compiler")
